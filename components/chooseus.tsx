...
import { db } from "@/lib/firebase"; // ✅ import db from your config file

const ChooseUs = () => {
  const { ref, inView } = useInView({ triggerOnce: true, threshold: 0.2 });
  const [hasAnimated, setHasAnimated] = useState(false);
  const [weeklyQuote, setWeeklyQuote] = useState<string>("");

  useEffect(() => {
    if (inView) setHasAnimated(true);
  }, [inView]);

  // Fetch weekly quote from Firebase Firestore
  useEffect(() => {
    const fetchQuote = async () => {
      try {
        const docRef = doc(db, "weekly", "weekly");
        const docSnap = await getDoc(docRef);
        if (docSnap.exists()) {
          const data = docSnap.data() as { text: string };
          setWeeklyQuote(data.text);
        } else {
          setWeeklyQuote("No quote found.");
        }
      } catch (error) {
        console.error("Error fetching weekly quote:", error);
        setWeeklyQuote("Error loading quote.");
      }
    };
    fetchQuote();
  }, []);
  ...
  const cards = [
    ...
    //   emoji: "📝",
    //   title: "Weekly Quote",
    //   desc: weeklyQuote || "Loading latest quote...",
    // },
  ];
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
        {cards.map((card, index) => (
          ...
          >
            ...
            <h3 className="text-xl font-semibold mb-2 tracking-wide">{card.title}</h3>
            <p className="text-sm text-center max-w-xs text-white/80">{card.desc}</p>
          </div>
        ))}
      </div>
    </section>
