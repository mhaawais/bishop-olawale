...
const Hero3 = () => {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setIndex((prev) => (prev + 1) % chunkedReviews.length);
    }, 6000);
    return () => clearInterval(interval);
  }, [chunkedReviews.length]);

  const next = () => setIndex((prev) => (prev + 1) % chunkedReviews.length);
  const prev = () =>
    setIndex(
      (prev) => (prev - 1 + chunkedReviews.length) % chunkedReviews.length
    );
        ...
        <div className="relative overflow-hidden">
          <div
            ...
            style={{
              ...
              width: `${chunkedReviews.length * 100}%`,
            }}
          >
            {chunkedReviews.map((group, idx) => (
              <div key={idx} className="flex w-full gap-4 px-2 flex-shrink-0">
                {group.map((r, i) => (
...
//           </h2>
//           <p className="text-gray-600 text-base sm:text-lg max-w-3xl mx-auto">
//             Real voices, real impact — how Bishop Olawale Olaofe’s books are transforming lives.
//           </p>
//         </div>

//         {/* Navigation Buttons */}
//         <div className="flex justify-center mb-8 gap-4">
//           <button onClick={prev} aria-label="Previous" className="p-3 bg-red-900 hover:bg-gray-800 rounded-full shadow-md">
//             <MdNavigateBefore size={26} className="text-white" />
//           </button>
//           <button onClick={next} aria-label="Next" className="p-3 bg-red-900 hover:bg-gray-800 rounded-full shadow-md">
//             <MdNavigateNext size={26} className="text-white" />
//           </button>
//         </div>
...
//                     {r.review}
//                   </p>
//                   <h3 className="font-bold text-[#7a1e21] text-sm sm:text-base">{r.name}</h3>
//                 </div>
//               </div>
//             ))}
//           </div>
